...
export default function ActiveDeals() {
  ...
  const deals = useMemo(
    () => allDeals.filter((d) => d.dealStage !== "closed_won" && d.dealStage !== "closed_lost"),
    [allDeals]
  );
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-400 mb-4">
        Deals in stages 0–4. Excludes closed-won and closed-lost.
      </p>

      {error && (
        <div className="bg-red-50 border border-red-200 text-red-600 rounded-2xl px-4 py-3 text-sm mb-4">{error.message}</div>
      )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#e5e5e5]">
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Last Contacted</th>
            ...
            <tbody className="divide-y divide-[#f0f0f0]">
              {isLoading ? (
                ...
                <tr><td colSpan={8} className="px-4 py-12 text-center text-gray-400">{search ? "No active deals match." : "No active deals."}</td></tr>
              ) : (
                deals.map((deal) => (
                  <tr key={deal.id} onClick={() => navigate(`/deals/${deal.id}`)} className="hover:bg-[#f9f9f9] cursor-pointer transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-400 tabular-nums">{formatDate(deal.lastContacted)}</td>
